...
class FraudDetectionApp {
    ...
    async submitTransaction() {
        ...
        if (!transactionId || !amount || !merchant || !location || !transactionTime || !merchantCategory) {
            this.showToast('Please fill in all fields including Merchant Category', 'warning');
            return;
        }
        ...
        try {
            ...
            const transactionData = {
                ...
                location: location,
                timestamp: transactionTime ? new Date(transactionTime).toISOString() : new Date().toISOString()
            };
    ...
    async analyzeTransaction(transactionData) {
        try {
            ...
            const apiData = {
                ...
                upi_id: "testuser@okaxis",                    // ← must have
                amount: parseFloat(transactionData.amount),
                merchant_id: transactionData.merchant || "MERCH_DEFAULT_123",
                merchant_category: transactionData.merchant_category || "ecommerce",   // ← now using selected value
                device_id: "DEV_TEST_" + Math.random().toString(36).slice(2,9),
                ip_address: "103.21.244.158",                  // Mumbai-like public IP
                location: {
                    latitude: 19.0760,
                    longitude: 72.8777,
                    accuracy: 15.0
                ...
                user_agent: navigator.userAgent || "Mozilla/5.0 (Test Frontend)",
                sms_content: `Rs.${transactionData.amount} paid to ${transactionData.merchant} on ${new Date(transactionData.timestamp).toLocaleString()}`,
                merchant_notes: "Test transaction from frontend"
            };
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                resolve({
                    ...
                    recommendation: this.getRecommendation(riskLevel) || "No recommendation available", // ← safety
    ...
    getRecommendationFromDecision(decision) {
        if (decision === 'ALLOW') {
            return 'Transaction appears safe. You can proceed normally.';
        }
        if (decision === 'CHALLENGE') {
            return 'Some risk indicators detected. Additional OTP verification is recommended.';
        }
        if (decision === 'BLOCK') {
            return 'High fraud risk. Recommend blocking this transaction or manual review.';
        }
        return 'No clear recommendation available at this time.';
    }
        ...
        <div class="result-content">
            <div class="result-summary">
                ...
                <div class="result-item">
                    ...
                    <span class="value">${result.confidence ? (result.confidence * 100).toFixed(1) + '%' : '90%'}</span>
                </div>
                <div class="result-item">
                    <span class="label">Processing Time:</span>
                    <span class="value">${result.processing_time_ms ? result.processing_time_ms.toFixed(1) : 15}ms</span>
                </div>
            </div>
            ...
            <div class="risk-factors">
                ...
                <div class="factors-list">
                    ${result.explanation?.human_readable 
                        ? `<p>${result.explanation.human_readable}</p>`
                        : '<p>No detailed explanation available from the model.</p>'}
                </div>
            </div>
    
            <div class="recommendation">
                <h4>Recommendation</h4>
                <p>${this.getRecommendationFromDecision(result.decision)}</p>
            </div>
    
            <!-- Removed the broken explanation block -->
            <!-- If you really want to show more details, use specific fields instead -->
        </div>
    `;
