...
def convert_numpy_scalars(obj):
    """Recursively convert NumPy scalars/arrays to Python natives for JSON serialization"""
    if isinstance(obj, np.generic):           # covers float32, int64, etc.
        return obj.item()
...
async def predict_fraud(transaction: TransactionRequest):
    ...
    try:
        ...
        return FraudResponse(
            ...
            explanation=convert_numpy_scalars(explanation),
            processing_time_ms=processing_time,
            features_used=list(features.keys()),
            alerts=alerts,
            model_details=convert_numpy_scalars({
